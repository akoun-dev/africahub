  ...
  "relative overflow-hidden border shadow-sm backdrop-blur-sm transition-all duration-300",
  {
    variants: {
      variant: {
        ...
        premium: "bg-white/99 border-gray-200/80 shadow-xl backdrop-blur-md",
        subtle: "bg-white/96 border-white/70 shadow-sm",
        // Nouveaux variants solides
        solid: "bg-white border-gray-200 shadow-lg",
        "solid-warm": "bg-gradient-to-br from-white to-amber-50/30 border-amber-200/70 shadow-lg",
        "solid-blue": "bg-gradient-to-br from-white to-blue-50/40 border-blue-200/70 shadow-lg",
        "african-card": "bg-gradient-to-br from-white via-amber-50/20 to-white border-amber-200/50 shadow-xl",
      },
      size: {
        default: "p-6",
        sm: "p-4",
...
const GlassCard = React.forwardRef<HTMLDivElement, GlassCardProps>(
  ({ className, variant, size, radius, withPattern = false, withHover = true, children, ...props }, ref) => {
    const isAfricanVariant = variant?.includes('african') || variant?.includes('warm')
