...
export const RoleBasedNavigation: React.FC = () => {
  ...
  const navigationConfig: Record<UserRole, Array<{
    label: string;
    path: string;
    ...
    merchant: [
      ...
      { label: 'Mes Produits', path: '/merchant/products', icon: Package },
      { label: 'Commandes', path: '/merchant/orders', icon: Store, badge: 3 },
      { label: 'Avis Clients', path: '/merchant/reviews', icon: MessageSquare },
      { label: 'Analytiques', path: '/merchant/analytics', icon: BarChart3 },
    ...
    manager: [
      ...
      { label: 'Produits', path: '/manager/products', icon: Package, badge: 23 },
    ...
    admin: [
      ...
      { label: 'Marchands', path: '/admin/merchants', icon: Store, badge: 8 },
  ...
  const getRoleBadge = (role: UserRole) => {
    const roleConfig = {
      ...
      merchant: { label: 'Marchand', color: 'bg-green-100 text-green-800' },
      manager: { label: 'Gestionnaire', color: 'bg-purple-100 text-purple-800' },
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center space-x-4">
            <Link to="/" className="flex items-center space-x-2">
              ...
              <span className="text-xl font-bold" style={{color: '#2D4A6B'}}>
                AfricaHub
              </span>
            </Link>
              ...
              return (
                <Link key={item.path} to={item.path}>
                  <Button
                    ...
                    size="sm"
                    className={`relative ${isActive ? 'text-white' : ''}`}
                    style={isActive ? {backgroundColor: '#2D4A6B'} : {}}
                  >
              ...
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="relative h-8 w-8 rounded-full">
                  <Avatar className="h-8 w-8">
                    ...
                    <AvatarFallback style={{backgroundColor: '#2D4A6B20', color: '#2D4A6B'}}>
              ...
              <DropdownMenuContent className="w-56" align="end" forceMount>
                ...
                {profile.role === 'merchant' && (
                  <>
                    ...
                    <DropdownMenuItem asChild>
                      <Link to="/merchant/analytics" className="cursor-pointer">
                ...
                {(profile.role === 'manager' || profile.role === 'admin') && (
                  <>
                    ...
                    <DropdownMenuItem asChild>
                      <Link to={`/${profile.role}/security`} className="cursor-pointer">
              ...
              return (
                <Link key={item.path} to={item.path}>
                  <Button
                    variant={isActive ? "default" : "ghost"}
                    size="sm"
                    className={`w-full relative ${isActive ? 'text-white' : ''}`}
                    style={isActive ? {backgroundColor: '#2D4A6B'} : {}}
                  >
                    <Icon className="w-4 h-4 mr-2" />
                    {item.label}
