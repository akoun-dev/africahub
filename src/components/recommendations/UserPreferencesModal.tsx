...
const UserPreferencesModal: React.FC<UserPreferencesModalProps> = ({
      ...
      if (data && !error) {
        ...
        if (data.coverage_priorities) {
          if (Array.isArray(data.coverage_priorities)) {
            coveragePriorities = data.coverage_priorities as string[];
          } else if (typeof data.coverage_priorities === 'string') {
            try {
              coveragePriorities = JSON.parse(data.coverage_priorities);
            } catch {
              coveragePriorities = [];
            }
          }
  ...
  const coveragePriorityOptions = {
    auto: ['liability', 'collision', 'comprehensive', 'medical', 'uninsured'],
    home: ['dwelling', 'personal_property', 'liability', 'additional_living', 'medical'],
    health: ['hospital', 'outpatient', 'prescription', 'dental', 'vision'],
    micro: ['crop', 'livestock', 'weather', 'income_protection', 'equipment']
  };
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        {loading ? (
          <div className="p-8 text-center">
            ...
            <p className="mt-2 text-gray-600">Chargement des préférences...</p>
          </div>
        ) : (
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="budget">Gamme de Budget</Label>
                    <Select
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="low">Économique (moins de 500€)</SelectItem>
                        <SelectItem value="medium">Moyen (500€ - 1500€)</SelectItem>
                        <SelectItem value="high">Premium (plus de 1500€)</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                ...
                <div>
                  <Label>Sensibilité au Prix: {(preferences.price_sensitivity * 100).toFixed(0)}%</Label>
                  <Slider
                    value={[preferences.price_sensitivity]}
                    onValueChange={([value]) => setPreferences(prev => ({ ...prev, price_sensitivity: value }))}
                    max={1}
                    min={0}
                    step={0.1}
                    className="mt-2"
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {(coveragePriorityOptions[insuranceType as keyof typeof coveragePriorityOptions] || []).map(priority => (
                    <Badge
                      key={priority}
                      variant={preferences.coverage_priorities.includes(priority) ? "default" : "outline"}
                      className="cursor-pointer justify-center py-2"
                      onClick={() => toggleCoveragePriority(priority)}
                    >
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <Label htmlFor="ai-recommendations">Recommandations IA</Label>
                    <p className="text-sm text-gray-600">Activer les recommandations basées sur l'IA</p>
                  </div>
                  <Switch
                    id="ai-recommendations"
                    checked={preferences.ai_recommendations}
                    onCheckedChange={(checked) => setPreferences(prev => ({ ...prev, ai_recommendations: checked }))}
                  />
                </div>

                <div className="flex items-center justify-between">
                  <div>
                    <Label htmlFor="real-time">Mises à jour en temps réel</Label>
                    <p className="text-sm text-gray-600">Recevoir les recommandations en temps réel</p>
                  </div>
                  <Switch
                    id="real-time"
                    checked={preferences.real_time_updates}
                    onCheckedChange={(checked) => setPreferences(prev => ({ ...prev, real_time_updates: checked }))}
                  />
                </div>

                <div>
                  <Label htmlFor="personalization">Niveau de Personnalisation</Label>
                  <Select
                    value={preferences.personalization_level}
                    onValueChange={(value) => setPreferences(prev => ({ ...prev, personalization_level: value }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      ...
                      <SelectItem value="high">Avancé</SelectItem>
                    </SelectContent>
                  </Select>
                </div>

                <div>
                  <Label htmlFor="notifications">Fréquence des Notifications</Label>
                  <Select
