...
export const RoleBasedRoute: React.FC<RoleBasedRouteProps> = ({
  ...
  if (!hasRequiredRole || !hasRequiredPermissions) {
    if (showAccessDenied) {
      return <AccessDeniedPage allowedRoles={allowedRoles} requiredPermissions={requiredPermissions} />;
    }
...
const AccessDeniedPage: React.FC<{
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{backgroundColor: '#2D4A6B10'}}>
      <Card className="w-full max-w-md text-center">
        <CardHeader>
          <div className="mx-auto mb-4 p-3 rounded-full" style={{backgroundColor: '#2D4A6B20'}}>
            <Shield className="w-8 h-8" style={{color: '#2D4A6B'}} />
          </div>
        ...
        <CardContent className="space-y-4">
          <div className="text-sm text-slate-600 space-y-2">
            ...
            {allowedRoles.length > 0 && (
              <div>
                <strong>Rôles autorisés :</strong> {allowedRoles.join(', ')}
              </div>
            )}
...
export const useRoleCheck = () => {
  const { hasRole, hasPermission, canAccess } = useEnhancedAuth();
...
}> = ({ children, allowedRoles = [], requiredPermissions = [], fallback = null }) => {
...
}> = ({ children, allowedRoles = [], requiredPermissions = [], disabledMessage }) => {
  const { checkAccess } = useRoleCheck();
