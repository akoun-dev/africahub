...
const ComparisonSection = () => {
  ...
  const [selectedCountry, setSelectedCountry] = useState<Country>(getAllAfricanCountries()[0]);

  // Use new hook that includes criteria values
  const { data: productsWithCriteria, isLoading } = useProductsWithCriteria(activeTab);

  const getFilteredProducts = (): ProductWithCriteria[] => {
    if (!productsWithCriteria) return [];
    
    return productsWithCriteria.filter(product => 
    ...
    if (selectedItems.includes(id)) {
      ...
      if (selectedItems.length < 3) {
        ...
        toast({
          ...
          description: "Vous pouvez comparer jusqu'à 3 offres à la fois",
          variant: "destructive",
        });
      }
    }
  ...
  const handleCompare = () => {
    if (selectedItems.length < 2) {
      toast({
        ...
        description: "Veuillez sélectionner au moins 2 offres pour les comparer",
        variant: "destructive",
      });
      return;
    }
    ...
    navigate(`/compare?type=${activeTab}&products=${selectedItems.join(',')}&country=${selectedCountry.name}`);
  };
      ...
      <div className="container px-4 md:px-6">
        <div className="flex flex-col items-center justify-center space-y-4 text-center mb-8">
          <div className="space-y-2">
            ...
            <p className="max-w-[900px] text-gray-500 md:text-xl/relaxed">
              Produits et services adaptés pour {selectedCountry.name} {selectedCountry.flag} et d'autres pays africains
            </p>
            
            <div className="flex justify-center mt-4 space-x-2">
              <div className="inline-flex items-center">
                <Globe className="h-5 w-5 mr-2 text-afroGreen" />
                <span className="font-medium">Pays sélectionné: {selectedCountry.name} {selectedCountry.flag}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="mb-8">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Nos suggestions IA basées sur votre profil et vos préférences
                </CardDescription>
              </CardHeader>
              <CardContent>
        ...
        <Tabs defaultValue="auto" value={activeTab} onValueChange={setActiveTab} className="w-full">
          <div className="flex justify-center mb-8">
            <TabsList className="kente-border">
              {sectors?.slice(0, 6).map(sector => (
                <TabsTrigger key={sector.slug} value={sector.slug}>
                  {sector.name}
                </TabsTrigger>
              ))}
            </TabsList>
          ...
          <div className="mb-6 text-center space-y-2">
            <Badge variant="secondary">
              {filteredProducts.length} produit{filteredProducts.length > 1 ? 's' : ''} disponible{filteredProducts.length > 1 ? 's' : ''} pour {selectedCountry.name}
            </Badge>
          ...
          {sectors?.map(sector => (
            <InsuranceTabContent 
              ...
              providers={activeTab === sector.slug ? filteredProducts : []} 
        ...
        <div className="mt-8 text-center">
          ...
          {selectedItems.length < 2 && (
            <p className="text-sm text-gray-500 mt-2">Sélectionnez au moins 2 offres pour les comparer</p>
          )}
        </div>
      </div>
