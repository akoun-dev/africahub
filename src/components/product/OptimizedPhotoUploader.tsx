    ...
    const handleFileSelect = useCallback(
        async (files: FileList) => {
            ...
            const filesToProcess = Array.from(files).slice(
                0,
                maxPhotos - photos.length - uploadingPhotos.length
            )
            ...
            for (const file of filesToProcess) {
                ...
                try {
                    ...
                    setUploadingPhotos(prev =>
                        prev.map(p =>
                            p.file === file
                                ? { ...p, status: "validating", progress: 95 }
                                : p
                        )
                    )
                    ...
                    if (isValid) {
                        ...
                        throw new Error(
                            "Photo rejetée par la modération automatique"
                        )
                    }
                } catch (error) {
    ...
    return (
        <div className="space-y-4">
            <Label className="text-sm font-medium">
                Photos (
                {photos.length +
                    uploadingPhotos.filter(p => p.status === "success").length}
                /{maxPhotos})
            </Label>
                ...
                >
                    <div className="space-y-2">
                        ...
                        <p className="text-xs text-gray-500">
                            JPG, PNG, WebP • Max {maxSizeKB / 1024}MB •
                    ...
                    {uploadingPhotos.map((uploadingPhoto, index) => (
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                {uploadingPhoto.error ? (
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        />
                                        <p className="text-xs text-gray-500">
                                            {uploadingPhoto.status ===
                                                "uploading" &&
                                                `Upload ${uploadingPhoto.progress}%`}
                                            {uploadingPhoto.status ===
                            ...
                            <Button
                                ...
                                size="sm"
                                onClick={() =>
                                    removeUploadingPhoto(uploadingPhoto.file)
                                }
                            >
                                <X className="h-4 w-4" />
                            </Button>
            ...
            {photos.length + uploadingPhotos.length >= maxPhotos && (
                <Alert>
                    ...
                    <AlertDescription className="text-sm">
                        Limite de {maxPhotos} photos atteinte. Supprimez une
