...
export const UserRoleManagement = () => {
    ...
    const [newUserRole, setNewUserRole] = useState<
        "admin" | "moderator" | "user" | "developer"
    >("user")
        ...
        try {
            ...
            for (const role of rolesData || []) {
                try {
                    ...
                    console.error(
                        "Error fetching user info for user:",
                        role.user_id,
                        error
                    )
                    rolesWithUserInfo.push({
        ...
        try {
            ...
            if (error) {
                if (error.message.includes("violates foreign key constraint")) {
                    toast.error(
                        "Utilisateur non trouvé. Utilisez l'UUID de l'utilisateur."
                    )
            ...
            <Card>
                <CardHeader>
                    ...
                    <p className="text-sm text-gray-600">
                        Note: Pour le moment, utilisez l'ID utilisateur UUID au
                        lieu de l'email
                    </p>
                </CardHeader>
