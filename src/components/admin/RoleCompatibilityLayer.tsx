...
export const RoleCompatibilityLayer: React.FC = () => {
  ...
  const migrateExistingRoles = async () => {
    ...
    try {
      ...
      if (!oldRoles || oldRoles.length === 0) {
        toast.success('Aucun rôle à migrer - tous les rôles sont déjà dans le nouveau système');
        setMigrationStatus('completed');
        return;
      }
      ...
      for (const oldRole of oldRoles) {
        ...
        if (roleId) {
          ...
          if (updateError) {
            console.error(`Erreur migration rôle ${oldRole.role}:`, updateError);
          } else {
            migratedCount++;
          }
        } else {
      ...
      toast.success(`Migration terminée: ${migratedCount} rôles migrés avec succès`);
      setMigrationStatus('completed');
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">
              Migration des rôles de l'ancien système vers le nouveau système paramétrable
            </p>
          </div>
        ...
        {migrationStatus === 'pending' && (
          <Alert>
            ...
            <AlertDescription>
              Des utilisateurs utilisent encore l'ancien système de rôles. 
              Cliquez sur "Migrer" pour les convertir vers le nouveau système paramétrable.
            </AlertDescription>
          </Alert>
        )}
        ...
        {migrationStatus === 'completed' && (
          <Alert>
            ...
            <AlertDescription>
              ✅ Excellent ! Tous les rôles ont été migrés vers le nouveau système paramétrable.
              Le système de récursion RLS a été corrigé et vous avez maintenant accès complet.
            </AlertDescription>
          </Alert>
        )}
