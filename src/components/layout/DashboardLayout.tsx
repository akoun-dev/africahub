...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({
  ...
  const { loading, profile } = useEnhancedAuth();
  ...
  if (!profile) {
    return (
      <div className="flex items-center justify-center min-h-screen" style={{backgroundColor: '#2D4A6B10'}}>
        <div className="text-center">
          <p className="text-slate-600">Aucun profil utilisateur trouvé</p>
        </div>
      </div>
    );
  }
        ...
        <RoleBasedContent 
          ...
          fallback={
            <div className="flex items-center justify-center min-h-[50vh]">
              <div className="text-center">
                <p className="text-slate-600">Accès non autorisé à cette section</p>
              </div>
            </div>
          }
        >
