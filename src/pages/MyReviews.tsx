...
export const MyReviews: React.FC = () => {
  ...
  const calculateStats = () => {
    ...
    const totalHelpfulVotes = userReviews.reduce((sum, review) => sum + (review.helpful_count || 0), 0);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-2xl font-bold">{stats.totalHelpfulVotes}</p>
                <p className="text-sm text-gray-600">Votes utiles</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Vues estimées</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Tabs defaultValue="all" className="space-y-6">
        <TabsList>
          <TabsTrigger value="all">Tous mes avis ({userReviews?.length || 0})</TabsTrigger>
          ...
          {userReviews && userReviews.length > 0 ? (
            userReviews.map((review) => {
              ...
              const typedReview: Review = {
                ...review,
                status: (review.status as 'active' | 'pending' | 'hidden' | 'deleted') || 'active'
              };
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <h4 className="font-medium">{review.products?.name || 'Produit'}</h4>
                            {review.products?.companies && (
                              <Badge variant="outline">
                                {review.products.companies.name}
                              </Badge>
                            )}
                          </div>
                      ...
                      <div className="flex gap-2">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleEditReview(typedReview)}
                        >
            ...
            <Card>
              <CardContent className="p-6 text-center">
                ...
                <p className="text-gray-600 mb-4">
                  Vous n'avez pas encore publié d'avis. Commencez par noter et commenter vos produits préférés !
                </p>
                <Button>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {review.products?.name} • {new Date(review.created_at).toLocaleDateString('fr-FR')}
                      </p>
                    </div>
                    <div className="flex">
                      {[...Array(5)].map((_, i) => (
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="font-medium">{review.title}</h4>
                      <p className="text-sm text-gray-600">
                        {review.products?.name} • {review.helpful_count} votes utiles
                      </p>
                    </div>
                    <div className="flex">
                      {[...Array(5)].map((_, i) => (
