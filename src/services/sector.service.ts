...
import {
    SectorRepository,
    type SectorRow,
    type SectorCreateResult,
    type SectorPartialUpdate,
} from "@/repositories/sector.repository"
import { MockDataService, type MockSector } from "./mockData.service"
...
export class SectorService {
     ...
     * Utilise des données fictives temporairement
     */
    static async getAllSectors(): Promise<SectorRow[]> {
        try {
            // Utilisation des données fictives au lieu de Supabase
            const mockSectors = await MockDataService.getAllSectors()
    ...
    static async getSectorBySlug(slug: string): Promise<SectorRow> {
        ...
        try {
            // Utilisation des données fictives au lieu de Supabase
            const mockSector = await MockDataService.getSectorBySlug(slug)
            return {
                id: mockSector.id,
                name: mockSector.name,
                slug: mockSector.slug,
                description: mockSector.description,
                icon: mockSector.icon,
                color: mockSector.color,
                is_active: mockSector.is_active,
                created_at: mockSector.created_at,
                updated_at: mockSector.updated_at,
            }
        } catch (error) {
            console.error(
                `Erreur lors de la récupération du secteur ${slug}:`,
                error
            )
    ...
    static async createSector(sectorData: unknown): Promise<SectorRow> {
        ...
        try {
            ...
            if (
                ...
                error.message !== "Secteur non trouvé"
            ) {
                throw error
            }
        }
        ...
        const result: SectorCreateResult = await SectorRepository.createSector(
            validatedData
        )
    ...
    static async updateSector(
        ...
        if (validatedUpdates.slug) {
            try {
                const existingSector = await SectorRepository.getSectorBySlug(
        ...
        try {
            ...
            const updateData: SectorPartialUpdate = {
                ...
                ...(validatedUpdates.icon !== undefined && {
                    icon: validatedUpdates.icon,
                }),
                ...(validatedUpdates.color !== undefined && {
                    color: validatedUpdates.color,
                }),
                ...(validatedUpdates.is_active !== undefined && {
                    is_active: validatedUpdates.is_active,
                }),
            }
    ...
    static validateSectorData(sector: SectorRow): boolean {
        return !!(sector.name && sector.slug && sector.color && sector.icon)
    }
