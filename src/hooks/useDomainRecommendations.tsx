...
export const useDomainRecommendations = (insuranceType?: string) => {
  ...
  const { withRetry, withFallback, handleError } = useRecommendationErrorHandler();
  const { 
  ...
  const recommendationService = Container.getInstance().get<IRecommendationService>('IRecommendationService');
  ...
  const fetchRecommendations = useCallback(async () => {
    ...
    try {
      const primaryOperation = async () => {
        return await recommendationService.getRecommendations(user.id, insuranceType);
      };
  ...
  }, [user, insuranceType, recommendationService, withFallback, handleError, getCachedRecommendations, setCachedRecommendations]);

  const generateRecommendations = useCallback(async (preferences: any, behavioralData?: any) => {
    if (!user) return;
  ...
  }, [user, insuranceType, recommendationService, withRetry, handleError, invalidateCache, fetchRecommendations]);
  ...
  const updateInteraction = useCallback(async (
    ...
    try {
      const operation = async () => {
        await recommendationService.updateInteraction(recommendationId, interactionType);
      };
      ...
      setRecommendations(prev => 
        prev.map(rec => 
          rec.id === recommendationId 
            ? { 
                ...rec, 
                isViewed: interactionType === 'viewed' || rec.isViewed,
                isClicked: interactionType === 'clicked' || rec.isClicked,
                isPurchased: interactionType === 'purchased' || rec.isPurchased
              }
            : rec
        )
      );
      ...
      if (user) {
        ...
        setCachedRecommendations(user.id, updatedRecommendations, insuranceType);
      }
    } catch (err) {
      handleError(err as Error, 'updateInteraction');
      setError(err instanceof Error ? err.message : 'Unknown error');
    }
  }, [recommendationService, withRetry, handleError, recommendations, user, setCachedRecommendations, insuranceType]);
