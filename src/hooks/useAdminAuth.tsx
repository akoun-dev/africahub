...
export const useAdminAuth = (): UseAdminAuthReturn => {
    ...
    const checkAdminStatus = useCallback(async () => {
        ...
        if (!user || !session) {
            console.log(
                "❌ useAdminAuth: No user or session found, clearing admin state"
            )
            setAdminUser(null)
            setIsAdmin(false)
            setLoading(false)
        ...
        try {
            console.log(
                "🔍 useAdminAuth: Checking admin status for user:",
                user.email
            )

            // Utiliser le nouveau système de rôles paramétrables avec gestion d'erreur améliorée
            const { data: userRoles, error } = await supabase.rpc(
                "get_user_roles_v2",
                {
                    _user_id: user.id,
                }
            ...
            if (error) {
                console.error(
                    "❌ useAdminAuth: Error fetching parametric roles:",
                    error
                )
            ...
            if (hasAdminRole || userRoles?.length > 0) {
                ...
                const roles =
                    userRoles?.map((role: any) => role.role_name) || []
                const permissions =
                    userRoles?.reduce((acc: string[], role: any) => {
                ...
                console.log(
                    "✅ useAdminAuth: Admin user verified with parametric roles:",
                    {
                ...
                console.log(
                    "❌ useAdminAuth: User has no admin roles:",
                    user.email
                )
            ...
            console.error(
                "💥 useAdminAuth: Unexpected error checking admin status:",
                error
            )
            setAdminUser(null)
            setIsAdmin(false)
    ...
    const hasPermission = useCallback(
        (permission: string): boolean => {
            ...
            if (
                ...
                adminUser.roles.includes("super-admin")
            )
                return true
