...
interface ProactiveAlert {
  id: string;
  alert_type: 'provider_degradation' | 'cost_spike' | 'latency_increase' | 'failure_pattern';
...
export const useLLMProactiveAlerts = () => {
    ...
    queryFn: async () => {
      ...
      if (hour >= 9 && hour <= 17) { // Business hours
        mockAlerts.push({
          ...
          message: 'La latence moyenne d\'OpenAI a augmenté de 45% par rapport à la baseline',
          provider: 'openai',
          metrics: {
            current_value: 2890,
      ...
      if (hour === 14) { // Afternoon spike simulation
        mockAlerts.push({
          id: `alert_${Date.now()}_2`,
          alert_type: 'cost_spike',
          severity: 'high',
          title: 'Pic de coûts détecté',
          message: 'Les coûts horaires ont dépassé le budget prévu de 120%',
          metrics: {
            current_value: 0.048,
  ...
  const applyAutoMitigation = useMutation({
    mutationFn: async ({ alertId, mitigationType }: { alertId: string; mitigationType: string }) => {
      console.log(`Applying auto-mitigation ${mitigationType} for alert ${alertId}`);
      
      // Simulate mitigation actions
      const mitigationActions = {
        'switch_to_cheap_provider': 'Basculement automatique vers DeepSeek',
        'enable_aggressive_cache': 'Cache agressif activé pour 2 heures',
        'reduce_model_temperature': 'Température réduite à 0.3 pour économiser les tokens',
        'circuit_breaker': 'Circuit breaker activé pour le provider défaillant'
      };
      ...
      return {
        success: true,
        action_applied: mitigationActions[mitigationType as keyof typeof mitigationActions] || 'Action inconnue'
      };
    },
    onSuccess: (result) => {
      toast({
      ...
      queryClient.invalidateQueries({ queryKey: ['llm-proactive-alerts'] });
    }
  });
