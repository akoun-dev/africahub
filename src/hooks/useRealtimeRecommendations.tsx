...
export const useRealtimeRecommendations = (insuranceType?: string) => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        const channel = supabase
          ...
          .on('broadcast', { event: 'recommendation_update' }, (payload) => {
            ...
            const newRecommendations = payload.payload.recommendations;
            setRecommendations(prev => {
              // Fusionner avec les recommandations existantes
              const merged = [...newRecommendations];
              const existingIds = new Set(newRecommendations.map((r: any) => r.id));
        ...
        console.error('Error initializing recommendations stream:', error);
        setStreamStatus('error');
        setLoading(false);
        handleReconnection();
      }
    ...
    const generateStreamingRecommendations = async () => {
      try {
        const { data, error } = await supabase.functions.invoke('stream-recommendations', {
          body: {
            user_id: user.id,
