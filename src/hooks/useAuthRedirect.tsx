...
export const useAuthRedirect = () => {
  ...
  useEffect(() => {
    ...
    if (authLoading || adminLoading) {
      console.log('🔄 useAuthRedirect: Waiting for auth/admin check to complete');
      return;
    }
    ...
    if (!user && location.pathname !== '/auth') {
      console.log('🔐 useAuthRedirect: User not authenticated, redirecting to /auth');
      navigate('/auth');
      return;
    }
    ...
    if (user && location.pathname === '/auth') {
      ...
      console.log('✅ useAuthRedirect: User authenticated, redirecting to:', redirectPath);
    ...
    if (user && isAdmin && location.pathname === '/dashboard') {
      console.log('👑 useAuthRedirect: Admin user on dashboard, redirecting to /admin');
      navigate('/admin');
      return;
    }
