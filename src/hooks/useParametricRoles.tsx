...
export const useParametricRoles = () => {
  ...
  useEffect(() => {
    const fetchUserRoles = async () => {
      ...
      try {
        console.log('🔍 useParametricRoles: Fetching roles for user:', user.id);
        
        const { data, error } = await supabase.rpc('get_user_roles_v2', {
          _user_id: user.id
        });

        if (error) {
          console.error('❌ useParametricRoles: Error fetching roles:', error);
          setRoles([]);
          setPermissions([]);
        } else {
          console.log('✅ useParametricRoles: Roles fetched successfully:', data);
          ...
          setRoles(userRoles.map((role: any) => ({
            id: role.role_id,
            name: role.role_name,
  ...
  const getHighestRole = () => roles.reduce((highest, current) => 
