...
export const useAnalyticsTracking = () => {
    ...
    const trackPageView = useCallback(
        async (page: string, sector?: string) => {
            await trackEvent({
                eventType: "page_view",
                sector,
                properties: {
    ...
    const trackProductView = useCallback(
        async (productId: string, sector: string, companyId?: string) => {
            await trackEvent({
    ...
    const trackComparison = useCallback(
        async (productIds: string[], sector: string) => {
            await trackEvent({
    ...
    const trackQuoteRequest = useCallback(
        async (productId: string, sector: string, amount?: number) => {
            await trackEvent({
    ...
    const trackSectorNavigation = useCallback(
        async (fromSector: string, toSector: string) => {
            await trackEvent({
